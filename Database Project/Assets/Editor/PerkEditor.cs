...
public class PerkEditor : EditorWindow {
    ...
    string perkIconPath;
    Texture2D perkIcon;
    List<int> perkConditions = new List<int>();
    int conditionID;
    #endregion
    ...
    void SaveNewPerk() {
        ...
        newPerk.PerkConditions = perkConditions;
    ...
    void SaveExistingPerk() {
        ...
        selectedPerk.PerkConditions = perkConditions;
    ...
    void GetPerkData() {
        ...
        perkConditions = selectedPerk.PerkConditions;
        perkIcon = EditorGUILayout.ObjectField("Icon: ", selectedPerk.GetIcon(), typeof(Texture2D), true) as Texture2D;
    }

    void DisplayPerkConditions() {
        EditorGUILayout.LabelField("Assigned Conditions", EditorStyles.boldLabel);
        EditorGUILayout.BeginHorizontal();
        // Display the currently-added conditions
        addedConditionScrollPos = EditorGUILayout.BeginScrollView(addedConditionScrollPos, false, false, GUILayout.MinWidth(270), GUILayout.Height(180));
        for (int i = 0; i < perkConditions.Count; i++)
        {
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("X", GUILayout.Width(50)))
            {
                perkConditions.RemoveAt(i);
                break;
            }
            EditorGUILayout.LabelField(_conditionDatabase.Condition(perkConditions[i]).ConditionName + " (ID " + _conditionDatabase.Condition(perkConditions[i]).ConditionID + ")");
            EditorGUILayout.EndHorizontal();
        }
        EditorGUILayout.EndScrollView();
        ...
        {
            int _id = _conditionDatabase.Conditions[j].ConditionID;
            string _name = _conditionDatabase.Conditions[j].ConditionName;

            if (perkConditions.Contains(_id))
                continue;
            ...
            if (GUILayout.Button("+", GUILayout.Width(50)))
            {
                ...
                {
                    perkConditions.Add(conditionID);
                    // now sort the list...
                }
        ...
        conditionID = EditorGUILayout.IntField("Condition ID: ", conditionID);
    ...
    bool ConditionRequirementsMet() {
        ...
        {
            Debug.LogError("Condition with ID " + conditionID + " does not exist in the database.");
            return false;
        }

        if (perkConditions.Contains(conditionID))
        {
